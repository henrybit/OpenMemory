...
if (is_pg) {
    ...
    const init = async () => {
        ...
        await pg.query(
            `create index if not exists idx_stats_ts on "${sc}"."stats"(ts)`,
        );
        await pg.query(
            `create index if not exists idx_stats_type on "${sc}"."stats"(type)`,
        );
        ready = true;
    };
    init().catch((err) => {
...
export const log_maint_op = async (
    ...
    try {
        const is_pg = env.metadata_backend === "postgres";
        const sc = process.env.OM_PG_SCHEMA || "public";
        const table_name = is_pg ? `"${sc}"."stats"` : "stats";
        const sql = is_pg
            ? `insert into ${table_name}(type,count,ts) values($1,$2,$3)`
            : `insert into ${table_name}(type,count,ts) values(?,?,?)`;
